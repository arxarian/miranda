					...
					if ( buf[0] ) {
						for (int i = 0; i < accounts.getCount(); ++i)
							if (_stricmp(buf, accounts[i]->szModuleName) == 0) {
								TCHAR buf[ 200 ];
								mir_sntprintf( buf, SIZEOF(buf), TranslateT( "Account error" ) );
								MessageBox( NULL, TranslateT( "Internal name must be unique. Please, enter different internal name." ), buf, MB_ICONINFORMATION | MB_DEFBUTTON1 | MB_OK );
								return FALSE;
							}
				}	}
